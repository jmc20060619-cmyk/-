...
function buildDashboardPayload(interestParam = "") {
...
function buildMonitorPayload(filterKey = "all", interestParam = "") {
  const stories = buildStories();
...
function buildWarningsPayload(interestParam = "") {
  const dashboard = buildDashboardPayload(interestParam);
...
function buildSearchPayload(query = "", interestParam = "") {
  const stories = buildStories();
...
async function handleApiRequest(requestUrl, response) {
  ...
  if (requestUrl.pathname === "/api/dashboard") { sendJson(response, 200, buildDashboardPayload(requestUrl.searchParams.get("interest") || "")); return; }
  if (requestUrl.pathname === "/api/monitor") { sendJson(response, 200, buildMonitorPayload(requestUrl.searchParams.get("filter") || "all", requestUrl.searchParams.get("interest") || "")); return; }
  if (requestUrl.pathname.startsWith("/api/events/")) { sendJson(response, 200, buildEventPayload(decodeURIComponent(requestUrl.pathname.replace("/api/events/", "")))); return; }
  if (requestUrl.pathname === "/api/events") { sendJson(response, 200, buildEventPayload(requestUrl.searchParams.get("id") || "")); return; }
  if (requestUrl.pathname === "/api/warnings") { sendJson(response, 200, buildWarningsPayload(requestUrl.searchParams.get("interest") || "")); return; }
  if (requestUrl.pathname === "/api/search") { sendJson(response, 200, buildSearchPayload(requestUrl.searchParams.get("query") || "", requestUrl.searchParams.get("interest") || "")); return; }
